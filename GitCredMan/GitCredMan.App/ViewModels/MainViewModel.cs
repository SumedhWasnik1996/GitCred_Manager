...
public partial class MainViewModel : ObservableObject
{
    ...
    // ── Nav / UI state ────────────────────────────────────────
    [ObservableProperty] private int     _activePage   = 0;   // 0=Accounts, 1=Repos
    ...
    public async Task ApplyToRepoAsync(Repository repo)
    {
        ...
        var token = _accountSvc.GetToken(account.Id);
    ...
    public async Task ScanAsync()
    {
        ...
        // Spinner rotation on UI thread via timer
    ...
    public string ResolveAccountName(Repository repo)
    {
        ...
        return specific ? acc.Name : $"{acc.Name}  (default)";
    }
    ...
    /// <summary>
    /// Import a discovered identity as a new Account.
    /// The user will be prompted to enter a token via the normal Add dialog,
    /// pre-populated with the discovered name/email/host.
    /// </summary>
    public Account BuildAccountFromDiscovered(DiscoveredIdentity identity)
    {
        return new Account
        {
...
// NOTE: the following partial additions extend MainViewModel.
// In the actual file these should be inside the class body.
// They are appended here for patch clarity — see instructions below.
